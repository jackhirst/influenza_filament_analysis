...
def calculate_eccentricity(major, minor):
    '''takes the major and minor axes of an ellipse and returns the eccentricity'''
...
def plot_bar(df, show_points = True, y_lim = 1.2, y_min = 0, make_stats=True,
             dpi = 600, figheight =5, figwidth = 12,
             x_title = "X", y_title = "Y", axis_fontsize = 12,
             tick_fontsize = 10, axis_weight = 2, edgecolor = "black",
             x_min = 0):
    set_bar_params()
...
def quick_lin_reg(df):
    y_vals = combine_lists(df["values"])
    x_vals = combine_lists([[float(x) for x in z] for z in df["scatter_values"]])
...
def plot_scatter(df, show_points = True, y_lim = 1.2,make_stats=True,
    ...
    plt.xlim(x_min, max(df["conditions"]) + extra_x_space)
    ...
    if line_type == "linear":
        ...
        popt, pcov = curve_fit(exp_func, list(df["conditions"]), list(df["means"]), maxfev = 1600)
        print("a = %s , b = %s, c = %s" % (popt[0], popt[1], popt[2]))
        many_xs = np.arange(0, max(df["conditions"]), 0.01)
